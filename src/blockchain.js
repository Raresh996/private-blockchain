...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            const ACCEPTED_DELAY_IN_SECONDS = 5 * 60;

            const messageTime = parseInt(message.split(':')[1]);
            const currentTime = parseInt(self._getCurrentTime());

            if(currentTime - messageTime > ACCEPTED_DELAY_IN_SECONDS) {
                reject(new Error("Delay is greater than accepted one"));
            }
            ...
            try {
                const newBlock = await self._addBlock(new BlockClass.Block({star: star, owner: address}));
                resolve(newBlock);
            } catch(err) {
                reject(err);
            }
        ...
        return new Promise((resolve, reject) => {
            self.chain.forEach(async (block) => {
                try {
                    ...
                    if(blockData && blockData.owner && blockData.owner === address) {
